...
ximc_dir = os.path.join(cur_dir, "ximc-2.13.3/ximc") # Formation of the directory name with all dependencies. The 
ximc_dir = r'C:\Program Files\XILab\ximc-2.13.3\ximc' #dependencies for the examples are located in the ximc directory.
...
# ximc_package_dir = 'C:\Program Files\XILab'
# sys.path.append(ximc_package_dir)  # add pyximc.py wrapper to python path
...
if platform.system() == "Windows":
    ...
    if sys.version_info >= (3,8):
        ...
        os.environ["Path"] = libdir + ";" + os.environ["Path"] # TIVNAN: add dll path into an environment variable
    else:
...
def test_status(lib, device_id):
    ...
    # print("Result: " + repr(result))
    if result == Result.Ok:
        print('Aperture intialized successfully.')
...
# # first move
# test_set_speed(lib, device_id, 2000)
# test_left(lib, device_id)
# time.sleep(3)
# endpos, uendpos = test_get_position(lib, device_id)
# # second move
...
# print("DEBUG ", endpos, uendpos)
# test_move(lib, device_id, endpos, uendpos)
# test_wait_for_stop(lib, device_id, 100)
# test_left(lib, device_id)
# time.sleep(3)
# test_move(lib, device_id, startpos, ustartpos)
...
class Aperture(object):
    def __init__(self,):
        ...
        # for p in ports:
        #     if 'XIMC Motor Controller' in p.description:        
        #         self.device_id = lib.open_device(('xi-com:'+p.device).encode())
        open_name = lib.get_device_name(devenum, 0)
        if type(open_name) is str:
            open_name = open_name.encode()
    ...
    def set_speed(self,speed):
        test_set_speed(lib, self.device_id, speed)
    def get_speed(self):
        speed = test_get_speed(lib, self.device_id)
        return speed
    def set_position(self,pos):
        pos = check_minmax(pos, self.limits[0], self.limits[1])
        test_move(lib, self.device_id, pos, 0)
        test_wait_for_stop(lib, self.device_id, 100)
    def get_position(self):
        startpos, ustartpos = test_get_position(lib, self.device_id)
        return startpos
    
    def get_limits(self):
        if platform.node() == 'imaging-lab-3':
            self.limits = [-104182, -102209]
        elif platform.node() == 'imaging-lab-1':
            self.limits = [-3950, -2002]
        elif platform.node() == 'imaging-lab-2':
            self.limits = [-631, 1295]
        return self.limits


# myRobotIris.set_speed(2000)
# myRobotIris.set_position(-2000)
# print(myRobotIris.get_position())
# print(myRobotIris.get_speed())
# time.sleep(2)
# myRobotIris.set_speed(2000)
# myRobotIris.set_position(-1500)
# print(myRobotIris.get_position())
# print(myRobotIris.get_speed())
# time.sleep(2)
# myRobotIris.set_speed(1000)
# myRobotIris.set_position(-1000)
# print(myRobotIris.get_position())
# print(myRobotIris.get_speed())
# time.sleep(2)
# myRobotIris.set_speed(1000)
# myRobotIris.set_position(-500)
# print(myRobotIris.get_position())
# print(myRobotIris.get_speed())
# time.sleep(2)
# myRobotIris.set_speed(500)
# myRobotIris.set_position(0)
# print(myRobotIris.get_position())
# print(myRobotIris.get_speed())
# time.sleep(2)
