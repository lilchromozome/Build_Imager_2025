...
class Camera:
    ...
    def open(self,  roi_shape=(1280, 1024), roi_pos=(0,0), exposure=10., framerate=1.0):
        ...
        _ = self.get_allowed_pixel_clock()
    ...
    def get_properties(self,):
        ...
        print('Allowed Pixel Clock', '\t', '[{}, {}]'.format(self.allowedpixelclock[0], self.allowedpixelclock[-1]), 'MHz')
    ...
    def live_image(self):
        # Live image display for fine-tuning the focus
        fig, ax = plt.subplots(figsize=(7, 7))
        plt.close(fig) 
        # live image display with exception handling
        try:
            while True: 
                ax.imshow(self.capture().copy(), clim=[0, 1023], cmap='gray')
